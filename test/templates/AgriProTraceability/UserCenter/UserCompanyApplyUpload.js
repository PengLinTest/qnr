var UserCompanyApplyUpload;
UserCompanyApplyUpload || (UserCompanyApplyUpload = {});
var Flash = {
    getFileData: function(a, c) {
        UserCompanyApplyUpload.addFile(c, "data:;base64," + a)
    }
}; (function() {
    UserCompanyApplyUpload = {
        files: [],
        fileIndex: 0,
        fileListUL: "",
        fileItemFormat: "",
            	...
            	$(c).on("change",
		            function(a) {
		                if (a.target.files && 0 != a.target.files.length){
		                	if (2 <= UserCompanyApplyUpload.files.length){
